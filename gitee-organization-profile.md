...
HNU-cloudcomputing 是湖南大学计算机学院云计算课程组维护的开放教学资源平台，主要服务于本科专业选修课《云计算技术》。平台集中发布课程主页、教材、课件、教学视频、示例代码、课程实验和自动测试。
...
为了帮助学生深入理解云计算系统中的网络通信、并发控制、分布式协同和云原生部署，我们围绕同一个 Go 语言文字游戏设计了四组逐步递进的实验。

实验一建立两个客户端与一个服务器之间的网络闭环；实验二将双人对战扩展为多人实时世界，处理同一进程内的并发访问；实验三将单地图、单节点的世界扩展为多地图、多节点系统，加入跨节点路由、事务、一致性和故障恢复；实验四把这套分布式游戏部署到 Kubernetes，在正确性、可用性、弹性和资源成本之间进行调优。

每组实验都提供可运行的程序骨架、实验说明和自动测试。学生在预留的 TODO 位置补充核心逻辑，通过测试、运行日志和故障模拟观察系统行为，再将运行结果与教材中的机制对应起来。
