...
这门课会手把手带你做一件事：从零写一个能真人对战的在线游戏，再亲手把它一路撑上云。

你将依次闯过五关—— **双雄对决** ：先写出双人对战的最小版本，搞定网络通信与状态同步，消息怎么切、延迟怎么压、TCP 和 UDP 怎么选； **英雄集结** ：玩家多起来了，把一台机器的并发榨干，协程、锁、I/O 多路复用、连接池，每优化一步都拿延迟和吞吐说话； **切分世界** ：单机撑不住了，把系统拆到多台机器，分片、复制、共识、容错，亲手按下一致性与可用性的取舍开关； **飞升入定** ：机器多到管不过来了，交给平台，容器化、Kubernetes 编排、弹性伸缩、故障自愈； **穷理尽微** ：最后沉到机制深处，看清容器、网络与调度的本来面目，学习核心原理，一直摸到容器加速和网络虚拟化的研究前沿。

这里没有要背的概念清单。每项技术出场，都是你上一轮亲手捅出的麻烦把它逼出来的；每个优化，都要跑出数字才算数。游戏跑起来就能真人对战，系统做得好不好，当场见分晓。

每个教学阶段均配有代码演示和递进式实验。学生需要运行系统，记录延迟、吞吐和资源占用，修改关键机制并比较结果。课程结束时，你能够分析云计算系统中的规模、状态、故障和资源问题，选择合适的机制，并通过实验验证设计方案。本课程适合具备基本编程能力和数据结构知识的学习者；接触过计算机网络和操作系统则更佳。
